...
// ------------------------------------------------------------------ unités ---
export function toUnit(ml, unit) { return unit === "oz" ? ml / ML_PER_OZ : ml; }
...
export function countBetween(feeds, from, to) {
  return feeds.filter((f) => t(f) >= from.getTime() && t(f) < to.getTime()).length;
}
...
/** Aujourd'hui jusqu'à maintenant, face à hier jusqu'à la même heure. */
export function compareToday(feeds, now) {
  const start = startOfDay(now), yStart = addDays(start, -1), yNow = new Date(now.getTime() - DAY);
  const inRange = (from, to) => feeds.filter((f) => t(f) >= from.getTime() && t(f) <= to.getTime());
  const today = inRange(start, now), yesterdaySoFar = inRange(yStart, yNow);
  const yesterdayAll = feeds.filter((f) => dayKey(f.started_at) === dayKey(yStart));
  return {
    today: { total: sum(today), count: today.length },
    yesterdaySoFar: { total: sum(yesterdaySoFar), count: yesterdaySoFar.length },
    yesterday: { total: sum(yesterdayAll), count: yesterdayAll.length },
  };
}
...
/** Heure probable du prochain boire : dernier boire + intervalle médian des 7 derniers jours. */
export function nextFeedEstimate(feeds, now) {
  const last = lastFeed(feeds, now);
  if (!last) return null;
  const recent = gaps(feeds.filter((f) => t(f) >= now.getTime() - 7 * DAY));
  if (recent.length < 4) return null;
  return new Date(t(last) + median(recent.map((g) => g.gap)));
}
