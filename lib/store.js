...
const TIMER_KEY = "kenda.timers.v1";
...
/** Minuteurs en cours (allaitement, tire-lait) : ils survivent à la fermeture
 *  de la fiche, de l'app et au verrouillage du téléphone. Un par genre.
 *  { babyId, startedAt (ISO, premier démarrage), sides: { left, right, all } en
 *    secondes accumulées, running: côté en cours ou null, since: ms, lastSide } */
export const timerMemory = {
  get(kind) { return read(TIMER_KEY, {})[kind] || null; },
  set(kind, t) { const all = read(TIMER_KEY, {}); all[kind] = t; write(TIMER_KEY, all); },
  clear(kind) { const all = read(TIMER_KEY, {}); delete all[kind]; write(TIMER_KEY, all); },
};
