...
/** File des boires à envoyer (ajouts, corrections, suppressions). Une seule
 *  entrée par boire : la version la plus récente remplace la précédente. */
export const queue = {
...
export function applyQueue(feeds) {
  const byId = new Map(feeds.map((f) => [f.id, f]));
  for (const q of queue.all()) {
